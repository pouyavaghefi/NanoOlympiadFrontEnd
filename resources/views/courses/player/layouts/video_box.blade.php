...
@if($play)
    @php
        if ($play->episode_number >= 2) {
            $currentPlayRoute = route('frt.crs.course_player.index.rest', ['slug' => $course->slug, 'epi' => $play->slug]);
        } else {
            $currentPlayRoute = route('frt.crs.course_player.index', ['slug' => $course->slug]);
        }
    @endphp
    <div class="col-xxl-7 col-xl-7 col-lg-8 col-md-12 order-1" id="scrollHere">
        ...
        <div class="animated-bg-wrapper">
            <div class="overlay"></div> <!-- readability overlay -->
