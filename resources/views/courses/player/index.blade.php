...
@php
    ...
    $comingUp = \App\Models\Episode::where('episode_number',$play_next)->first();
    if($play->episode_number >= 2){
        $play_prev_option = true;
        $play_prev = $play->episode_number - 1;
        $lastOne = \App\Models\Episode::where('episode_number',$play_prev)->first();
...
@section('scripts')
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        Swal.fire({
                            ...
                            preConfirm: () => {
                                if (nextSlug) {
                                    window.location.href = `{{ route('frt.crs.course_player.index.rest', ['slug' => $course->slug, 'epi' => $nextSlug]) }}`;
                                }
                            }
                        });
                    } else {
    ...
    <script>
        // Check if the video fails to load or play
        video.addEventListener('error', function() {
            console.error("Video failed to load or an error occurred.");
            // Optionally show a SweetAlert or other UI feedback
            Swal.fire({
                icon: 'error',
                title: 'Video Error',
                text: 'There was an issue loading the video. The page will refresh.',
            }).then(() => {
                location.reload(); // Refresh the page to retry loading the video
            });
        });
